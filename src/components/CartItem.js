...
const mapDispatchToProps = (dispatch,ownProps) =>{
  return {
    remove:()=>dispatch(removeItem(ownProps.id)),
    // remove:()=>dispatch({type:'REMOVE_ITEM',payload:{id:ownProps.id}}),
    increase:()=>dispatch({type:'INCREASE_ITEM',payload:{id:ownProps.id}}),
    decrease:()=>dispatch({type:'DECREASE_ITEM',payload:{id:ownProps.id,amount:ownProps.amount}}),
  }
}
